...
exports.UaiRangoService = void 0;
const axios_1 = __importDefault(require("axios"));
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
class UaiRangoService {
    constructor() {
        this.AUTH_URL = 'https://merchant-api.uairango.com/authentication/v1.0/oauth/token';
    }
    async getValidToken(empresaId, config) {
    ...
    async autenticar(config) {
        ...
        try {
            const { data } = await axios_1.default.post(this.AUTH_URL, params.toString(), {
                headers: {
    ...
    async salvarTokenNoBanco(empresaId, authData, configAtual) {
        ...
        await prisma.empresa.update({
            where: { id: empresaId },
            data: { configUaiRango: novaConfig }
        });
    }
    async buscarEventosPendentes(empresaId, config) {
    ...
    async confirmarRecebimento(empresaId, config, eventIds) {
        const token = await this.getValidToken(empresaId, config);
    ...
    async salvarPedidoNoBanco(tenantId, evento) {
        try {
    ...
    async buscarPedidoPorId(empresaId, config, orderId) {
        const token = await this.getValidToken(empresaId, config);
        try {
    ...
    async getPedidoDetalhes(tenantId, pedidoId, token) {
        try {
            const response = await axios_1.default.get(`https://merchant-api.uairango.com/order/v1.0/orders/${pedidoId}`, {
                headers: {
...
exports.UaiRangoService = UaiRangoService;
